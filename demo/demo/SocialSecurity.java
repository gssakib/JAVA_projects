	...
	public static void main(String[] args) {
		...
		if(social.length() == 11){
			...
			for (int i = 0 ; i < 11; i++ ){
				...
				char aChar = social.charAt(i); //indexing through each character
				//System.out.println(aChar);
				if ( i != 3 && i != 6){  // if its not indexing where the hyphen is supposed to be
					if (aChar != "-".charAt(0)){//if the character is not a hyphen
						//System.out.println(aChar);
				...
				}else{// if the character is a hyphen and it is indexed where its supposed to be
					//System.out.println(aChar);
					if(aChar == "-".charAt(0)){
						//System.out.println(aChar);
						count = count + 1;
